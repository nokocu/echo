...
public class AuthController : ControllerBase
{
    ...
    private async Task CreateDefaultProjectForUser(string userId)
    {
        ...
        // Create default BPMN workflow for the project
        var defaultWorkflow = new BpmnWorkflow
        {
            Name = "Default Task Workflow",
            Description = "Default BPMN workflow for task management",
            ProjectId = project.Id,
            OwnerId = userId,
            BpmnXml = BpmnUtils.GenerateDefaultBpmnXml("Default Task Workflow"),
            BpmnJson = "{}",
            IsActive = true,
            Version = 1
        };

        _context.BpmnWorkflows.Add(defaultWorkflow);
        await _context.SaveChangesAsync();
    }
}
